...
3. 普通审查修复以额外 conventional integration-fix commits 普通 push，保持 stage branch 追加式前进。`/pr-ai-review-loop` 需要 rebase 或 force-push 时停止写入并把独占写权交回 team-lead；team-lead 将 stage rebase 到最新 `origin/main`，解决冲突、运行累计质量门，再用 `--force-with-lease` 更新远程。保留每个 issue 的单个 conventional commit 与 `Refs #<N>`，按 [handoff.md](handoff.md) 记录新旧 HEAD 后重新交出写权，从新 HEAD 重启本循环。
